...
public class TenPinBowling {
    ...
    public static int bowling_score(String frames) {
        ...
        for (int i = 0; i < 9; i++) {
            totalScore += getScore(framesArr[i], i, framesArr);
        }

        totalScore += getLastFrameScore(framesArr[9]);

        return totalScore;

    }

    private static int getScore(String currentFrame, int index, String[] framesArr) {
        if (currentFrame.equals("X")) {
            return 10 + getStrikeBonusPoints(framesArr[index + 1], framesArr, index + 1);
        } else if (currentFrame.contains("/")) {
            return 10 + getSpareBonusPoints(framesArr[index + 1]);
        }

        int firstRollScore = Integer.parseInt(currentFrame.split("")[0]);
        int secondRollScore = Integer.parseInt(currentFrame.split("")[1]);
        return firstRollScore + secondRollScore;
    }

    private static int getStrikeBonusPoints(String nextFrame, String[] framesArr, int index) {
        if (nextFrame.startsWith("X")) {
            if (index == 9) {
                String lastFrame = framesArr[index];
                if (lastFrame.substring(1, 2).equals("X")) {
                    return 20;
                }
                return 10 + Integer.parseInt(lastFrame.substring(1, 2));
            }
            String nextNextFrame = framesArr[index + 1];
            if (nextNextFrame.startsWith("X")) {
                return 20;
            }
            return 10 + Integer.parseInt(nextNextFrame.substring(0, 1));
        } else if (nextFrame.substring(0, 2).contains("/")) {
            return 10;
        }
        int firstRollScore = Integer.parseInt(nextFrame.split("")[0]);
        int secondRollScore = Integer.parseInt(nextFrame.split("")[1]);
        return firstRollScore + secondRollScore;
    }

    private static int getSpareBonusPoints(String nextFrame) {
        if (nextFrame.startsWith("X")) {
            return 10;
        }
        return Integer.parseInt(nextFrame.split("")[0]);
    }

    private static int getLastFrameScore(String frame) {
        int score = 0;
        String[] lastFrameArr = frame.split("");
        for (int i = 0; i < lastFrameArr.length; i++) {
            if (lastFrameArr[i].equals("X")) {
                score += 10;
            } else if (lastFrameArr[i].equals("/")) {
                score += 10;
                score -= Integer.parseInt(lastFrameArr[i - 1]);
            } else {
                score += Integer.parseInt(lastFrameArr[i]);
            }
        }
