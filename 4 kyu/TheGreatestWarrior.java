//Create a class called TheGreatestWarrior which calculates and keeps track of their level and skills, and ranks them as the warrior they've proven to be.
//
//Business Rules:
//
//A warrior starts at level 1 and can progress all the way to 100.
//A warrior starts at rank "Pushover" and can progress all the way to "Greatest".
//The only acceptable range of rank values is "Pushover", "Novice", "Fighter", "TheGreatestWarrior", "Veteran", "Sage", "Elite", "Conqueror", "Champion", "Master", "Greatest".
...
public class TheGreatestWarrior {
    ...
    public TheGreatestWarrior() {
        this.xp = 100;
        this.level = RankingSystem.getLevel(this.xp);
        this.rank = RankingSystem.getRank(this.level);
    ...
    public String battle(int enemyLevel) {
        ...
        if (this.level == enemyLevel) {
            ...
            increaseSkill(5);
            return "A good fight";
        } else if (this.level - 2 >= enemyLevel) {
            return "Easy fight";
        } else if (this.level < enemyLevel) {
            int difference = enemyLevel - this.level;
            if (difference >= 5 && !RankingSystem.getRank(this.level).equals(RankingSystem.getRank(enemyLevel))) {
                return "You've been defeated";
            }
            increaseSkill(20 * difference * difference);
        }
        return "An intense fight";
    }

    public String training(String description, int experience, int levelRequired) {
        if (this.level < levelRequired) {
            return "Not strong enough";
        }
        increaseSkill(experience);
        this.achievements.add(description);
        return description;
    }

    private void increaseSkill(int experience) {
        int newXp = this.xp + experience;
        this.xp = newXp > 10000 ? 10000 : newXp;
        this.level = RankingSystem.getLevel(this.xp);
        this.rank = RankingSystem.getRank(this.level);
    }
}
...
class RankingSystem {
    ...
    protected static int getLevel(int xp) {
        return xp / 100;
    }

    protected static String getRank(int level) {
